...
def test_narrator_voice_direction_and_character_override(tmp_path):
    job = DubJob(tmp_path / "movie.mkv", "en", "es", source_audio=tmp_path / "source.wav")
    job.segments = [Segment(0, 0, 2, "Hello", speaker="NARRATOR")]
    job.speakers = {"NARRATOR": Speaker("NARRATOR")}
    calls = []

    class Voicebox:
        def synthesize_to_file(self, profile, text, language, dest, **kwargs):
            calls.append((profile, kwargs.get("instruct")))
            dest.parent.mkdir(parents=True, exist_ok=True)
            dest.write_bytes(b"audio")

    vb = Voicebox()
    synthesize.run(
        job, vb, tmp_path, narrator_voice="warm", narrator_delivery="Calm", engine="qwen"
    )
    assert calls == [("warm", "Calm")]
    synthesize.run(
        job, vb, tmp_path, narrator_voice="warm", narrator_delivery="Calm", engine="qwen"
    )
    assert len(calls) == 1
    synthesize.run(
        job, vb, tmp_path, narrator_voice="deep", narrator_delivery="Quiet", engine="qwen"
    )
    assert calls[-1] == ("deep", "Quiet")
    synthesize.run(
        job,
        vb,
        tmp_path,
        narrator_voice="deep",
        narrator_delivery="Quiet",
        engine="qwen",
        cast={"NARRATOR": {"voice": "character", "delivery": "Urgent"}},
    )
    assert calls[-1] == ("character", "Urgent")


def test_single_character_repair_does_not_inherit_narrator_voice(tmp_path):
    job = DubJob(tmp_path / "movie.mkv", "en", "es", source_audio=tmp_path / "source.wav")
    job.segments = [Segment(0, 0, 2, "Hello", speaker="CHARACTER")]
    job.speakers = {"CHARACTER": Speaker("CHARACTER", voicebox_profile_id="actor")}
    calls = []
    ...
    synthesize.run(job, Voicebox(), tmp_path, narrator_voice="narrator", narrator_speakers=set())
    assert calls == ["actor"]
