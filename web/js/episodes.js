  ...
  function render() {
    ...
    root.innerHTML = `<div class="episode-toolbar"><div><h3>Episodes</h3>
      ...
      ${rows.filter(e => e.season === season).map(e => `<div class="episode-row" data-episode="${e.id}">
        ...
        <div class="episode-name"><span class="m">S${String(e.season).padStart(2, '0')}E${String(e.episode).padStart(2, '0')}</span><strong>${esc(e.title)}</strong><span class="hint">${e.audio_langs.length ? e.audio_langs.map(l => esc(l.toUpperCase())).join(' · ') : 'No audio metadata'}</span></div>
