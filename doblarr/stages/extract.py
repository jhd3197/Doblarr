...
def _select_audio_stream(job: DubJob, cancel=None) -> int:
    data = json.loads(run_ffprobe([
        "-v", "error", "-select_streams", "a", "-show_entries",
        "stream=index:stream_tags=language", "-of", "json", str(job.input_file)],
        cancel=cancel))
...
def run(job: DubJob, work_dir: Path, dry_run: bool = False,
    ...
    identity = {"input": str(job.input_file.resolve()), "stream": index, "duration": duration}
    hit = cached(out, job.input_file, force)
    if hit and receipt.exists() and json.loads(receipt.read_text()) == identity:
        return hit
